...
/*
    NOTE: This version is Modified to better handle noise on the comms line
*/
...
//ac{}
  ...
  extern void LogHex(byte rx);
  extern void LogHex(char* s);
  extern void LogHex(byte* s , int len);
//ac{}
...
void SoftwareSerial::setRxGPIOPinMode() {
    if (m_rxValid) {
        Serial.printf("Setting pin %d to be input. Pullup %d\n",m_rxPin, hasRxGPIOPullUp(m_rxPin) && m_rxGPIOPullUpEnabled );
...
void SoftwareSerial::begin(uint32_t baud, SoftwareSerialConfig config,
    ...
    Serial.printf("m_pduBits: %d  m_bitTicks: %d m_dataBits: %d m_parityMode:%d\n",m_pduBits,m_bitTicks,m_dataBits,m_parityMode);
...
void SoftwareSerial::rxBits() {
    ...
    m_isrBuffer->for_each(m_isrBufferForEachDel); //calls self->rxBits(isrTick); for each isrTick in m_isrBuffer
    ...
    //]if (m_rxLastBit!=9 && m_pduBits != 10 )
    //]    Logf("m_rxLastBit=%d m_pduBits=%d\n",m_rxLastBit, m_pduBits); //idle we have 9,10

    //if (m_rxLastBit < m_pduBits + 2) {
    if (m_rxLastBit < m_pduBits - 1) {
        ...
        if (!m_isrBuffer->available() && microsToTicks(micros()) - m_isrLastTick > detectionTicks) {
            ...
            //Log("[F!]");
            //]]rxBits((m_isrLastTick + detectionTicks) | 1);  //think must be a 1
...
/*
static String dbgbuf = "";
//this is called out of interrupt to parse the captured Time+Levels
void SoftwareSerial::rxBits(const uint32_t isrTick) {
    //]Logf("Out: %x\n", isrTick);
    const bool level = (m_isrLastTick & 1) ^ m_invert; //LAST TICK - and 5v is "0"
    ...
    Logf("(%d x %d)", bits, level);

    digitalWrite(DAC2, level); //show current level on scope
    ...
    while (bits > 0) {
        ...
        if (m_rxLastBit >= (m_pduBits - 1)) {
            ...
            if (level){
                Log("[xS]");
                //Ignore drop - Need wait for End of the Stop pulse
                bits=0;                      //ac{}
                break; //this throws away all the built up bits since last byte
            }
            digitalWrite(DAC1, HIGH); //signal end of stopbit()
            m_rxLastBit = -1;
            --bits;
            dbgbuf="S";
            Log("[S]");
        ...
        {//end of Byte... going to dump all bits left over.
            digitalWrite(DAC1, LOW);

            if (!level)
            {
                Log("[xE]"); //wrong Stop bit level...
            }
            else
            {
                Log("[E]");
...
void SoftwareSerial::rxBits(const uint32_t isrTick) {
    ...
    Serial.printf("%d,%d,", ticks, level); //this is one that runs
...
void SoftwareSerial::rxBits(const uint32_t isrTick) {
    ...
    m_isrLastTick = isrTick;    //we need store this as the level needs to change next tick

    if (ticks < 300)
        return; //ticks = 1206; ; //remove glitch - dont update m_isrLastTick
...
//here every level change - push time and level for later processing
void IRAM_ATTR SoftwareSerial::rxBitISR(SoftwareSerial* self) {
    ...
    //#ifdef dbgBuf
    //if (digitalRead(DAC1) == HIGH)
    //{//Show only as long as we are capturing this packet
    //    const uint32_t v = level ? (curTick | 1U) : (curTick & ~1UL);
    //    self->m_isrBuffer->push(v);
    //    Logf("In: %x\n", v);
    //}
    //#else
        const uint32_t v = level ? (curTick | 1U) : (curTick & ~1UL);
    ...
    /*while((curTick+200) > microsToTicks(micros()))
    {}
    digitalWrite(DAC1, LOW);
    */
